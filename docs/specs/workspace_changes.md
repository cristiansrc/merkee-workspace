...
| TD-MSF-STAB-001 | `active`. JWT default inseguro en `jwt.adapter.ts` (`process.env.JWT_SECRET \|\| 'merkee-shop-dev-secret-change-in-production'`); `.env.example` no documenta `JWT_SECRET`. Riesgo de seguridad en producción. | Bloqueante para producción; se cierra con STAB-B3 (fail-fast en producción + documentar `.env.example`). |
| TD-MSF-STAB-002 | `active`. ROP drift en `activate-admin.use-case.ts` (`try/catch` técnico en `application`); `ActivateAdminUnitOfWorkPort.run` devuelve `Promise<T>` (no `Result`) y el adapter no traduce excepciones. Contradice Master Spec §ROP y el patrón de provision/bootstrap. | Bloqueante para revalidación; se cierra con STAB-B1 (puerto `Result`, adapter traduce, application sin `try/catch`). |
| TD-MSF-STAB-003 | `active`. `must_change_password` drift en replay de provisión: OpenAPI declara `const: true`; el código replay devuelve `user.mustChangePassword` (dinámico) y `schemas.ts` tipa `boolean`. Decisión STAB-DEC-01 Opción A: replay devuelve `true`. | Bloqueante para revalidación; se cierra con STAB-B2 (forzar `true` en replay + tipar literal `true` en `schemas.ts`). |
| TD-MSF-STAB-004 | `active`. ROP JWT verify drift. `JwtAdapter.verify` propaga excepciones de `jwt.verify` (`TokenExpiredError`/`JsonWebTokenError`/`NotBeforeError`) sin traducir a `DomainError`; `JwtPort.verify` devuelve `Promise<JwtPayload>` (no `Result`); el controller `identity.controller.ts` inyecta `JwtAdapter` concreto (no `JwtPort`) y captura la excepción con `try/catch`. Contradice Master Spec §ROP y ADR-017. | Bloqueante para revalidación; se cierra con STAB-B5 (`JwtPort.verify`→`Result`, `JwtAdapter.verify` traduce a `DomainError` del catálogo existente `AUTHENTICATION_REQUIRED` 401/`TECHNICAL_DEPENDENCY_FAILURE` 500, controller consume `Result` del puerto sin `try/catch`, DI actualizada). |
...
3. TD-MSF-STAB-001/002/003/004 (deudas de estabilización) se cierran con las correcciones de código STAB-B3/B1/B2/B5 respectivas; no se declaran resueltas sin la evidencia de cierre registrada en `technical_debt.md`.
